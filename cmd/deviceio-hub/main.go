...
var (
	...
	startAPIBind            = startCommand.Flag("api-bind", "The hostname or ip address to bind the http api to").Default("").String()
	startAPIPort            = startCommand.Flag("api-port", "The tcp port to use when binding the http api").Default("4431").Int()
	...
	startGatewayBind        = startCommand.Flag("gateway-bind", "The hostname or ip address to bind the gateway to").Default("").String()
